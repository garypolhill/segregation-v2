globals [
  ...
  spatial-autocor  ; store the Moran's I so we only calculate it once per tick
  similar-backup   ; store the %-similar-wanted parameter so we remember it
  ran-education?
]
...
to setup
  ...
  set similar-backup %-similar-wanted
  update-turtles
  update-globals
  reset-ticks
end
...
;  if all? turtles [ happy? ] [ stop ]
  ...
  update-globals
  if mouse-down? [
    ask [patches in-radius 5] of patch mouse-xcor mouse-ycor [
      ask turtles-here [ die ]
      set habitable? false
      set pcolor grey
    ]
  ]
  if spatial-autocor > target-moran-i [
    set %-similar-wanted %-similar-wanted * education-effect-% / 100
    set ran-education? true
  ]
  tick
end
...
to find-new-spot
  rt random-float 360
  fd random-float 10
  if any? other turtles-here or not [habitable?] of patch-here [ find-new-spot ] ; keep going until we find an unoccupied patch
...
to update-turtles
  ask turtles [
    ...
    set other-nearby count (turtles-on patches in-radius general-area-radius) with [ color != [ color ] of myself ]
    set total-nearby similar-nearby + other-nearby
    set happy? similar-nearby >= (%-similar-wanted * total-nearby / 100) and other-nearby >= (%-different-wanted * total-nearby / 100)
    if any? neighbors4 with [not habitable?] [
      set happy? false
    ]
...
to update-globals
  ...
  set spatial-autocor moran-i
  if %-similar-wanted < similar-backup [
    set %-similar-wanted min (list similar-backup (%-similar-wanted + education-forgetting-rate))
  ]
  set ran-education? false
end
...
to-report patch-x
  ifelse any? turtles-here [
    report mean [ color ] of turtles-here
  ] [
    report 0
  ]
end
...
to-report moran-i
  let x-bar mean [patch-x] of patches

  let numerator 0
  let denominator 0
  ask patches [
    let my-x patch-x
    set denominator denominator + (my-x - x-bar) ^ 2
    ask other patches [
      if member? myself neighbors [
        set numerator numerator + (my-x - x-bar) * (patch-x - x-bar)
      ]
    ]
  ]
  report 0.125 * numerator / denominator
end
@#$#@#$#@
GRAPHICS-WINDOW
375
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot spatial-autocor"
...
163
Reset %sw
set %-similar-wanted similar-backup
NIL
1
T
OBSERVER
